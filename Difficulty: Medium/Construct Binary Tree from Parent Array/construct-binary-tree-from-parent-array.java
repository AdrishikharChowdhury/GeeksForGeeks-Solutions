...
class Solution {
    // Changed method signature to match the driver code expectation
    static Node createTree(int parent[]) {
        int N = parent.length;  // Get the length of the array
        
        // Create an array to store all tree nodes
        ...
        // Connect nodes according to parent array
        for (int i = 0; i < N; i++) {
            if (parent[i] == -1) {
                root = nodes[i];
            } else {
